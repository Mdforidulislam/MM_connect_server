...
export type TSearchOption = 'exact' | 'partial' | 'enum' | 'search' | "IN" | undefined;
...
search(searchableFields: string[]) {
  ...
  // helper function to build nested condition
  const buildNestedCondition = (path: string, term: string): Record<string, any> => {
    const keys = path.split(".");
    const lastKey = keys.pop() as string;

    let condition: Record<string, any> = { [lastKey]: { contains: term, mode: "insensitive" } };
  ...
  if (searchTerm) {
    const orConditions = searchableFields.map((field) => {
      if (field.includes(".")) {
        return buildNestedCondition(field, searchTerm);
      } else {
